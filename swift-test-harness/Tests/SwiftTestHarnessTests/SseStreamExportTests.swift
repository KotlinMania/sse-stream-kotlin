...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `SseStream.swiftmodule/`
...
//      `import SseStream` above would fail at compile time.
//
//   2. The static archive `libSseStream.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "SseStream" }` and
//      `flattenPackage = "io.github.kotlinmania.ssestream"` configuration in
...
//      via the `SseStreamLibrary` product.
//
...
final class SseStreamExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "SseStream swift module imported cleanly")
    }
}
