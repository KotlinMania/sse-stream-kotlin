# Claude Code Project Instructions — sse-stream-kotlin

## Project Overview

This is **sse-stream-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`sse-stream`](https://crates.io/crates/sse-stream).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/sse-stream/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/sse-stream>
